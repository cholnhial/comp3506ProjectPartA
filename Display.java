...
class DisplayPartiallySorted extends DisplayPartiallySortedBase {
    ...
    @Override
    Plane[] sort() {
        Plane[] extraPlanes = Utils.mergeSort(super.getExtraPlanes(), 0, super.getExtraPlanes().length - 1);
        Plane[] fullySorted;
...
class Utils {
    ...
    public static Plane[] mergeArrays(Plane[] first, Plane[] second) {
        Plane[] result = new Plane[first.length + second.length];
        ...
        for (int i = 0; i < result.length - first.length; i++) {
            result[j] = second[i];
            j++;
        }
